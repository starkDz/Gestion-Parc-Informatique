...
async DeleteThis(id,index){
  try{
    ...
    const res = await axios.delete(url+'/api/document/'+id,{headers: {
...
render(){
    const { error, isLoaded, items,Title ,selectedRow,open,id,opensnack,msg,type} = this.state;
    if (error) {
      ...
      return <div><Loading_Item /></div>;
        ...
        return (
          ...
          <MaterialTable
            ...
            title={Title}
            columns={[
              { title: 'Numéro', field: 'num_document',width:"8%" },
              { title: 'Type', field: 'code_type_doc.description_Fr',width:"8%" },
              { title: 'Mouvement', field: 'code_type_mouv.description_Fr',width:"8%" },
              { title: 'Service Dest', field: 'code_service_dest.description_Fr',width:"20%" },
            ...
            actions={[
              ...
              },
              {
                ...
                onClick: (event,rowData) => {this.handleClickOpen();this.setState({id:rowData._id}); }
              }
            ]}
