...
const Sns_Serie = require('../../models/Sns_Serie');
...
router.get('/',async (req,res)=>{
    try {
        const types = await Sns_Serie.find().populate('owner',['name']);
        res.json(types);
    } catch (err) {
        console.error(err.message);
        res.status(500).send('Server Error');
...
router.delete('/:type_id',auth,async (req,res)=>{
    try {
        ...
        //remove type
        await Sns_Serie.findOneAndRemove({_id:req.params.type_id});
