...
const Sns_Ref = require('../../models/Sns_Ref');
...
router.get('/',async (req,res)=>{
    try {
        const types = await Sns_Ref.find().populate('owner',['name']);
        res.json(types);
    } catch (err) {
        console.error(err.message);
        res.status(500).send('Server Error');
...
router.delete('/:type_id',auth,async (req,res)=>{
    try {
        ...
        //remove type
        await Sns_Ref.findOneAndRemove({_id:req.params.type_id});
